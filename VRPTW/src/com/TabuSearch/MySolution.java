...
public class MySolution extends SolutionAdapter{
	...
	public void initializeRoutes(Instance instance) {
		...
		// We build up a matrix of routes: for each depot we have a list of archs. Eventually for our problem it's a vector matrix.
		for (int i = 0; i < instance.getDepotsNr(); ++i)
			for (int j = 0; j < instance.getVehiclesNr(); ++j){
					// initialization of routes
					routes[i][j] = new Route();
					routes[i][j].setIndex(i*(instance.getVehiclesNr()) + j); // Generating an index and assign it to the route element.
					...
					// assign vehicle, setting up contraints, as capacity of veichles and max duration of the round trip.
	...
	public void buildInitialRoutes1(Instance instance) {
		...
		// For each depot:
		// 1.   Set starting client # randomly
		// 2.   For each client assigned to the depot, scann all the routes (veichles)
		// 2.a    If a veichle associated to the route has enough space to serve the client and it has still time to serve him...
		// 2.b	  	In that case invoke InsertBestTravel, which will check if the new customer has to be inserted in head or in tail or even into the middle; this will also update the route accordingly.
		// 2.c      Then we calculate the route cost, simply eveluating the time to travel from one customer to the other adding relative service times.
		// 2.d      Then we break; so only the first feasible possibility is choosen
		...
		for( int i = 0; i < instance.getDepotsNr(); ++i) {
			...
			// If we dind't set up a customer to start the route from, decide one randomly.
			...
			for (int j = startCustomer; j < assignedCustomersNr + startCustomer; ++j) {
				...
				customerChosen = j % assignedCustomersNr; // Rotating
